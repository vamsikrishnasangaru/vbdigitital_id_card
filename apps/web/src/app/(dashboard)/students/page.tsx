...
export default function StudentsPage({ params }: NextClientPageProps) {
  ...
  const generateMutation = useMutation({
    mutationFn: async ({
      ...
      const ids = studentIds ?? visibleStudents.map((s: { id: string }) => s.id);
      ...
      if (result.kind === 'file') {
        ...
        if (result.failCount > 0) {
          ...
          const failedNames = result.failures?.length
            ? formatFailedStudentLabels(result.failures, visibleStudents)
            : '';
          toast.warning(
            `Downloaded ${result.successCount} of ${visibleStudents.length} card(s); ${result.failCount} failed${failedNames ? `: ${failedNames}` : ''}`,
            {
              duration: 20000,
              action:
                failedIds.length > 0
        ...
        if ((data.failCount ?? 0) > 0) {
          ...
          const failedNames = driveFailures.length
            ? formatFailedStudentLabels(driveFailures, visibleStudents)
            : '';
    ...
    <div className={cn('space-y-8', !skipPageEnterAnimation && 'animate-in fade-in duration-200')}>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="mt-0.5 text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">
            Students
          </h2>
        </div>
        <div className="flex w-full flex-col items-stretch gap-2 sm:flex-row sm:items-center md:w-auto">
          {effectiveSchoolId && isSuperAdmin && (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-emerald-700 disabled:opacity-50"
            >
              {generateMutation.isPending ? (
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-muted disabled:opacity-50"
          >
