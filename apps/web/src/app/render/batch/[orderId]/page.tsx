...
function BatchRenderPageContent() {
  const { orderId } = useParams();
  const searchParams = useSearchParams();
  const token = searchParams.get('token'); // In case we add auth later
  ...
  // Split cards into pages of 8 (2 columns x 4 rows)
  const pageSize = 8;
  const pages = [];
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {pages.map((pageCards, pageIndex) => (
        <div key={pageIndex} className="page-break p-[10mm] w-[210mm] h-[297mm] mx-auto border-b last:border-0 border-gray-200">
