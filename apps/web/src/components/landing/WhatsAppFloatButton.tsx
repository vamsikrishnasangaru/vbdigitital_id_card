...
const floatBtnClass =
  'flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg ring-4 ring-white/90 transition-transform hover:scale-105 sm:h-[3.75rem] sm:w-[3.75rem]';
...
export function WhatsAppFloatButton() {
  ...
  return createPortal(
    <div className="fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-4 z-[9999] flex flex-col gap-3 sm:right-6">
      <button
        type="button"
        onClick={openContact}
        aria-label="Contact us by email"
        className={`${floatBtnClass} bg-primary shadow-primary/30 hover:opacity-90`}
      >
        <Mail className="h-6 w-6 sm:h-7 sm:w-7" strokeWidth={2.25} />
      </button>
      <a
        ...
        className={`${floatBtnClass} bg-[#25D366] shadow-emerald-900/30 hover:bg-[#20bd5a]`}
      >
